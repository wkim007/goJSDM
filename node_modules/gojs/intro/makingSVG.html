...
<!-- Copyright 1998-2023 by Northwoods Software Corporation. -->    <title> GoJS Making SVG -- Northwoods Software </title>
...
<p>
  <b>GoJS</b> has one function for creating SVG: <a>Diagram.makeSvg</a>, which returns a new SVGElement with a representation of a GoJS Diagram.
  The method has a single argument, a JavaScript Object that contains several definable properties, enumerated in the documentation.
</p>
<p>
  SVG export can be useful as content for a PDF, because the rendering is vector oriented instead of raster oriented.
...
<p class="box bg-info">
  <a>Diagram.makeSvg</a> produces a static SVG "image" -- the SVG will not be interactive.
  Note that exporting a diagram to SVG is different from the functionality introduced in v2.3
  that allows the interactive diagram to be rendered in SVG rather than in Canvas.
  For more information please read: <a href="SVGContext.html">SVG Drawing Context</a>
</p>
...
<p>
  As of 2.3.11, GoJS makeSVG and the <a href="SVGContext.html">SVG Rendering Context</a> both set some <a target="_blank" href="https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/data-*">SVG data attributes</a>. These are:
</p>

<ul>
  <li><code>portId</code> if a <a>GraphObject.portId</a> is set.
  <li><code>itemIndex</code> if a <a>Panel.itemIndex</a> is set.
  <li><code>className</code> if a Part.
  <li><code>key</code> if a Part, if <a>Part.key</a> is set.
  <li><code>fromKey</code> if a Link with a fromNode, the <a>Part.key</a> of that Node.
  <li><code>fromPortId</code> if a <a>Link.fromPortId</a> is set.
  <li><code>toKey</code>  if a Link with a toNode, the <a>Part.key</a> of that Node.
  <li><code>toPortId</code> if a <a>Link.toPortId</a> is set.
</ul>
